...
2. 各指摘の妥当性を評価する
   ...
   - 実装上のトレードオフとして妥当か
3. 親が妥当と判断した指摘のみ逐次修正し、変更に応じた手動確認と自動テストを行う。修正はsubagentへ委譲しない
4. 修正diffを見直し、「修正時のコメント規約」に違反するコメントを追加していないかセルフチェックし、違反があれば取り除く
5. 最終サマリーを出力する
...
レビュー指摘への修正で、コメント規約に違反するコメントを混入させないこと。

- レビュー対応の経緯・修正理由・指摘への言及をコードコメントに書かない。それらは最終サマリーやコミットメッセージに書く
- 新たにコメントを追加してよいのは、コメント規約（公開APIの説明・コードから読めない背景情報・複雑なロジックの説明・100行以上の大きなファイルでのセクション分け）に該当する場合のみ
