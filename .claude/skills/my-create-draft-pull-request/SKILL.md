...
- そのリポジトリでの自分のPRの書き方をお手本として学ぶ。できるだけ似た種別のPRを優先的に集める:
    1. 類似PRを優先的に探す (best effort): 今回の変更種別に関連するキーワード（機能名、変更ファイルの特徴的な識別子など）を使って、自分（@me）のマージ済みPRを検索する。例: `ghro pr list --author @me --state merged --search "<キーワード>" --limit 3`。`--search` はPR title/bodyのテキスト検索なので、そこにキーワードが乗らないPRは拾えない点に注意。結果から今回と似ているものを最大3件ピックアップする。1〜2件しか見つからなくても、空振りでも、取れたぶんを最優先のお手本として扱い、足りなければ (2) にフォールバックする
    2. 不足分を最新マージから補充: 類似PRが1件以下の時のみ、不足分を `ghro pr list --author @me --state merged --limit <必要件数>` で最新マージ済みから補充する（合計3件を目安に）
    3. 自分のPRが1件以下の場合のみ、他の人のPRも含めて集める
...
- **書かないもの**:
    ...
    - 主張を際立たせない具体の指定（件数・規模などの数値、名前の列挙、括弧での言い換え・補足）。自己チェック: その具体を削って読み返したとき、PRで伝えたい主張が弱まるか？ 弱まらないなら削る
        - OK: 主張を構成する判断基準・閾値（「30秒以下の個別指定を削除」の30秒以下 — 削除対象を決めた基準そのもの）、主張の説得力を担う具体値（性能改善がコアのPRでの「実行時間短縮（20分→8分）」、「実測2秒で完走したので削除」の実測2秒）、伝えたいことの中身そのものの列挙（「PlanType（FREE/PRO/ENTERPRISE）ごとの検証に絞る」— 種別ごとの検証がコアである場合。PR説明は今この時点を表せばよいので、コードコメントと違い将来の変更での陳腐化を気にせず列挙してよい）
        - NG: 作業規模の集計（「30秒以下の個別指定88件を削除」の88件 — 削除方針が伝われば件数は主張に関与しない）、diffで分かる細部の逐一列挙（「不要なセットアップ（期待値の組み立て、外部サービスのモック、時刻モック等）を削除」）、念のための経緯の詳述（「（テスト側に暫定の回避処理を入れて対応していた）」→「#1234で入れた暫定対応」で足りる）
