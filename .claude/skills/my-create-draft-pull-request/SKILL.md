...
- **書かないもの**:
    ...
    - 低情報な括弧での言い換え・補足。括弧で情報を足してよいのは、このPRで伝えたいことを明確に際立たせる時だけ
        - OK: 伝えたいことに直結する具体値（性能改善がコアのPRでの「実行時間短縮（20分→8分）」）、伝えたいことの中身そのものの列挙（「PlanType（FREE/PRO/ENTERPRISE）ごとの検証に絞る」— 種別ごとの検証がコアである場合。PR説明は今この時点を表せばよいので、コードコメントと違い将来の変更での陳腐化を気にせず列挙してよい）
        - NG: diffで分かる細部の逐一列挙（「不要なセットアップ（期待値の組み立て、外部サービスのモック、時刻モック等）を削除」）、念のための経緯の詳述（「（テスト側に暫定の回避処理を入れて対応していた）」→「#1234で入れた暫定対応」で足りる）
